...
async function getApprovalsDetail(id: string) {
  try {
...
export default async function ApprovalsDetailPage({
  ...
  const approvals = await getApprovalsDetail(id);
  if (!approvals) return notFound();
  ...
  return (
    <div className="p-6 border rounded-xl bg-white shadow-sm max-w-4xl mx-auto mt-6">
      ...
      <h2 className="text-2xl font-bold mb-3">{approvals.title}</h2>
      <div className="flex items-center text-sm text-gray-500 mb-6 pb-4 border-b gap-4">
        <div className="flex items-center gap-1">
          ...
          <span>{approvals.userName}</span>
        </div>
        <div className="w-[1px] h-3 bg-gray-300"></div>
        <div>{new Date(approvals.createdAt).toLocaleString()}</div>
      </div>
      <div
        className="prose max-w-none whitespace-pre-wrap"
        dangerouslySetInnerHTML={{ __html: approvals.content }}
      />
      {approvals.fileUrl && (
        <div className="mt-8 pt-4 border-t">
          <p className="text-sm font-semibold mb-2">첨부파일</p>
          <a
            href={approvals.fileUrl}
            target="_blank"
            className="text-blue-600 hover:underline"
          >
            {approvals.fileName || "다운로드"}
          </a>
        </div>
      )}
    </div>

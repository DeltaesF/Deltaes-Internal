...
// API Response 타입
interface ApprovalDetailResponse extends Partial<SalesFormData> {
  id: string;
  approvalType: string;
  title: string;
}
...
export default function PurchaseApprovalEdit() {
  ...
  const [formData, setFormData] = useState<SalesFormData | null>(null);
    ...
    mutationFn: async () => {
      ...
      const res = await fetch("/api/approvals/update", {
        ...
        body: JSON.stringify({
          ...
          userName,
          approvalType: "sales",
          title: `[판매품의] ${formData.customerName}_${formData.product}`,
        }),
      });

  ...
  return (
    <div className="flex flex-col w-full p-4 md:p-6 min-w-0">
      <div className="bg-white border rounded-2xl shadow-sm p-4 md:p-6">
        {/* 상단 헤더 영역: 모바일에서 세로 배치가 가능하도록 gap과 flex 조절 */}
        ...
        {/* 리스트 영역: 일일 보고서 및 내부 보고서와 동일한 반응형 리스트 스타일 적용 */}
        {list.length === 0 ? (
          ...
          <>
            <ul className="w-full">
              {list.map((item) => (
                ...
                >
                  ...
                  >
                    {/* 왼쪽 영역: 제목이 길어지면 줄여주고 사이드바 공간 확보 */}
                    <div className="flex items-center gap-3 overflow-hidden flex-1 min-w-0">
                      <span className="text-[#519d9e] font-bold whitespace-nowrap shrink-0 text-sm md:text-base">
                        [외부]
                      </span>
                    ...
                    {/* 오른쪽 영역: 작성자와 날짜 (공간 부족 시 shrink 보호 및 모바일 대응) */}
                    <div className="flex items-center gap-3 md:gap-4 text-xs md:text-sm text-gray-500 shrink-0">
                      {/* 데스크톱 이상에서 보이는 작성자 이름 */}
                      <span className="hidden sm:inline-block font-semibold text-gray-700 bg-gray-100 px-2 py-1 rounded">
                        {item.userName}
                      </span>
                      {/* 모바일에서 글자수가 많을 경우를 대비한 처리 */}
                      <span className="sm:hidden font-semibold text-gray-700 bg-gray-100 px-2 py-0.5 rounded max-w-[60px] truncate">
                        {item.userName}
                      </span>
            ...
            {/* 페이지네이션 버튼: 태블릿/모바일 터치 환경에 최적화된 패딩 */}
